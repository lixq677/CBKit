...
@implementation UIImage (LXUnitls)
...
+ (LXImageType)imageTypeFromData:(NSData *)data{
    LXImageType type = LXImageTypeNone;
...
+ (UIImage *)localizedImageNamed:(NSString *)imageName inBundle:(NSBundle *)bundle{
    ...
    NSString *localizedImageName = [imageName stringByAppendingFormat:@"_%@",language];
    UIImage *image = [UIImage imageNamed:localizedImageName inBundle:bundle];
    if (image == nil) {
        image = [UIImage imageNamed:imageName inBundle:bundle];
    }
    return image;
}
