...
@implementation UIView (LXAdapt)

- (void)adaptScreenWidthWithType:(LXAdaptScreenWidthType)type
                     exceptViews:(NSArray<Class> *)exceptViews {
    if (type == LXAdaptScreenWidthTypeNone)  return;
    if (![self isExceptViewClassWithClassArray:exceptViews]) {
        
        // 是否要对约束进行等比例
        BOOL adaptConstraint = ((type & LXAdaptScreenWidthTypeConstraint) || type == LXAdaptScreenWidthTypeAll);
        
        // 是否对字体大小进行等比例
        BOOL adaptFontSize = ((type & LXAdaptScreenWidthTypeFontSize) || type == LXAdaptScreenWidthTypeAll);
        
        // 是否对圆角大小进行等比例
        BOOL adaptCornerRadius = ((type & LXAdaptScreenWidthTypeCornerRadius) || type == LXAdaptScreenWidthTypeAll);
